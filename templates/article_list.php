...
?>
    ...
    <?php for ( $i = 1; $i <= $page_count; $i ++ ) : ?>
        <?php if ($i == $page) : ?>
            <?php echo $i ?>
        <?php else: ?>
            <a href="/feed/<?php echo $feed->id ?>/articles?page=<?php echo $i ?>"><?php echo $i ?></a>
        <?php endif ?>
    <?php endfor ?>
    ...
    <a style="margin-left:20px" href="/feed/<?php echo $feed->id ?>/read?feed=<?php echo $feed->id ?>&page=<?php echo $page ?>&ids=<?php echo $article_ids ?>">&times; Mark Page Read</a>
    <a style="margin-left:20px" href="/feed/<?php echo $feed->id ?>/read?feed=<?php echo $feed->id ?>&page=1&ids=all">&times;&times; Mark All Read</a>
    <?php endif ?>
    </div>
<?php
    // Capture pager
