...
class Router
{
    ...
    public function getUrl( $name, $args = array(), $prefixed = true )
    {
        if( TRUE !== array_key_exists($name, $this->routes) )
            throw new NamedPathNotFoundException("URL not found for path \"$name\"");
     ...
     */
    public function findRoute( $path, $request_method='GET' )
    {
        foreach( $this->routes as $route )
        {
            if( TRUE === $route->matchMap( $path, $request_method ) )
            {
                return $route;
            }
        }
